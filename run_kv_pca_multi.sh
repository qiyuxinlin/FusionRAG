...
#####################################################################
# 多方法 KV Cache PCA 对比分析脚本
#
# 使用方法：
#   bash run_kv_pca_multi.sh <方法1> <方法2> [方法3 ...] [样本数] [输出目录]
#
# 示例：
...
SCRIPT_PATH="./visualize_kv_pca_multi.py"
...
if [ -z "$OUTPUT_DIR" ]; then
    ...
    OUTPUT_DIR="./kv_pca_multi_${METHODS_STR}"
fi
...
if [ ${EXIT_CODE} -eq 0 ]; then
    ...
    echo ""
    echo "生成的可视化文件："
    echo "  1. pca_key_example*_chunk*_*.png  - Key cache PCA 对比图"
    echo "  2. pca_value_example*_chunk*_*.png - Value cache PCA 对比图"
