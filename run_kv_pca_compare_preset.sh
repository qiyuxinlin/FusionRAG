...
#####################################################################
# KV Cache PCA 预设对比分析脚本
#
# 提供常用的对比预设，方便快速分析
#
# 使用方法：
#   bash run_kv_pca_compare_preset.sh <preset> [样本数] [输出目录]
#
# 可用预设：
...
#
#   # 对比3种ablation方法，分析10个样本
#   bash run_kv_pca_compare_preset.sh ablation3 10
#
#   # 对比所有方法，保存到指定目录
#   bash run_kv_pca_compare_preset.sh ablation_all 5 ./full_comparison
#####################################################################
...
case $PRESET in
    ...
    ablation_all)
        ...
        [ -z "$OUTPUT_DIR" ] && OUTPUT_DIR="./pca_ablation_all"
        ;;
    ...
    random_methods)
        ...
        [ -z "$OUTPUT_DIR" ] && OUTPUT_DIR="./pca_random_methods"
        ;;
    ...
    bge_variants)
        ...
        [ -z "$OUTPUT_DIR" ] && OUTPUT_DIR="./pca_bge_variants"
        ;;
...
echo ""
echo "调用: bash run_kv_pca_multi.sh ${METHODS} ${NUM_SAMPLES} ${OUTPUT_DIR}"
echo ""

# 调用主脚本
bash /home/shm/document/exp/FusionRAG/run_kv_pca_multi.sh ${METHODS} ${NUM_SAMPLES} ${OUTPUT_DIR}
