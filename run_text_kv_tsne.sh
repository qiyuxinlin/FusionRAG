...
#####################################################################
# 文本段落 KV Cache t-SNE 可视化脚本
#
# 使用方法：
#   bash run_text_kv_tsne.sh <选项>
#
# 示例：
#   # 直接在命令行指定文本
#   bash run_text_kv_tsne.sh --texts "这是第一个段落。" "这是第二个段落。"
#
#   # 从文件读取文本
#   bash run_text_kv_tsne.sh --text_file /home/shm/document/exp/FusionRAG/example_texts.txt --all_layers
#
#   # 指定GPU和输出目录
#   bash run_text_kv_tsne.sh --texts "Text 1" "Text 2" --device cuda:0 --output_dir ./results
#
#   # 指定要分析的层和perplexity
#   bash run_text_kv_tsne.sh --texts "Text 1" "Text 2" --layers 0 5 10 15 20 27 --perplexity 30
#
# 注意：t-SNE比PCA慢，每个样本每层需要1-2秒
#####################################################################
...
SCRIPT_PATH="./visualize_text_kv_tsne.py"
...
MAX_TOKENS=50000
PERPLEXITY=30
OUTPUT_DIR="./text_kv_tsne_analysis"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --model_type)
            ...
            ;;
        -h|--help)
            echo "用法: bash run_text_kv_tsne.sh [选项]"
            echo ""
            echo "选项:"
            ...
            echo ""
            echo "📌 t-SNE vs PCA:"
            echo "  • t-SNE: 更适合保留局部结构和发现聚类（较慢）"
            echo "  • PCA: 更快，保留全局结构，有方差解释率"
            exit 0
            ;;
        *)
...
if [ ${EXIT_CODE} -eq 0 ]; then
    ...
    echo ""
    echo "生成的可视化文件："
    echo "  1. tsne_key_cache.png   - Key cache t-SNE 可视化图"
    echo "  2. tsne_value_cache.png - Value cache t-SNE 可视化图"
