#!/usr/bin/env python3
"""
t-SNE Visualization of KV Cache Features Across Different Layers and Methods

This script analyzes and visualizes the feature distribution of Key and Value vectors
from KV cache across different layers, comparing no_preprocess and BGE modes using t-SNE.

t-SNE (t-Distributed Stochastic Neighbor Embedding) is better at preserving local structure
compared to PCA, making it useful for visualizing high-dimensional data clusters.

Usage:
    python visualize_kv_tsne.py --sample_ids 0 1 2 --max_layers 28 --perplexity 30
"""
...
class KVCacheTSNEAnalyzer:
    """Analyzer for KV Cache feature distribution using t-SNE"""
    ...
    def compute_tsne(self, features: np.ndarray, n_components: int = 2,
        ...
        Args:
            ...
            n_components: Number of t-SNE components (usually 2)
        ...
        print(f"t-SNE completed with perplexity={actual_perplexity}")
    ...
    def analyze_single_sample(self, example_id: int, chunk_id: int,
                            ...
                            perplexity: int = 30):
        """
        Analyze KV cache for a single sample using t-SNE
        ...
        for layer_idx in selected_layers:
            ...
            # Compute t-SNE
            print("Computing t-SNE for Key features...")
            key_tsne = self.compute_tsne(all_key_features, n_components=2, perplexity=perplexity)

            print("Computing t-SNE for Value features...")
            val_tsne = self.compute_tsne(all_val_features, n_components=2, perplexity=perplexity)

            # Split back
            n_samples = no_prep_key_feat.shape[0]
            no_prep_key_tsne = key_tsne[:n_samples]
            bge_key_tsne = key_tsne[n_samples:]

            no_prep_val_tsne = val_tsne[:n_samples]
            bge_val_tsne = val_tsne[n_samples:]
            ...
            results['layers'][layer_idx] = {
                ...
                'key_tsne': {
                    'no_prep': no_prep_key_tsne,
                    'bge': bge_key_tsne,
                },
                'val_tsne': {
                    'no_prep': no_prep_val_tsne,
                    'bge': bge_val_tsne,
                },
            }
    ...
    def plot_layer_comparison(self, results: Dict, output_dir: str):
        """
        Plot t-SNE visualization for all layers
        ...
        for idx, layer_idx in enumerate(layers):
            ...
            no_prep_key = layer_data['key_tsne']['no_prep']
            bge_key = layer_data['key_tsne']['bge']
            ...
            ax_key.set_xlabel('t-SNE Dim 1')
            ax_key.set_ylabel('t-SNE Dim 2')
            ...
            no_prep_val = layer_data['val_tsne']['no_prep']
            bge_val = layer_data['val_tsne']['bge']
            ...
            ax_val.set_xlabel('t-SNE Dim 1')
            ax_val.set_ylabel('t-SNE Dim 2')
        ...
        fig_key.suptitle(f'Key Cache t-SNE - Example {example_id}, Chunk {chunk_id}', fontsize=16, y=0.995)
        fig_val.suptitle(f'Value Cache t-SNE - Example {example_id}, Chunk {chunk_id}', fontsize=16, y=0.995)

        fig_key.tight_layout()
        fig_val.tight_layout()

        # Save figures
        key_path = output_path / f"tsne_key_example{example_id}_chunk{chunk_id}.png"
        val_path = output_path / f"tsne_value_example{example_id}_chunk{chunk_id}.png"
...
def main():
    parser = argparse.ArgumentParser(description='Visualize KV Cache using t-SNE across layers')
    ...
    parser.add_argument('--perplexity', type=int, default=30,
                       help='t-SNE perplexity parameter (5-50 typical range)')
    parser.add_argument('--output_dir', type=str, default='./kv_tsne_analysis',
                       help='Output directory for plots')

    args = parser.parse_args()

    # Initialize analyzer
    analyzer = KVCacheTSNEAnalyzer(args.cache_dir, args.dataset, args.model_name)
    ...
    for example_id in args.sample_ids:
        try:
            results = analyzer.analyze_single_sample(
                example_id, args.chunk_id, selected_layers, args.max_tokens, args.perplexity
            )
