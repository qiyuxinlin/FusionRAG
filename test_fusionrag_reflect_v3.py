...
class RecallMethod(Enum):
    ...
    SELF_SUPERVISED: Self-supervised mode - repeat task using existing KV cache
    """
    BGE = "bge"
    RANDOM = "random"
    ...
    SELF_SUPERVISED = "self_supervised"  # Self-supervised repeat task mode
...
def parse_repeat_generation(generated_text: str) -> str:
    """
    清理自监督重复任务的生成文本
    ...
    """
    # 移除可能的标记
    text = generated_text.split('</content>')[0] if '</content>' in generated_text else generated_text
    text = text.split('END')[0] if 'END' in text else text
    text = text.strip()
    return text


def compute_repeat_f1(original_text: str, generated_text: str, tokenizer) -> float:
    """
    计算重复任务的 F1 分数
    ...
    """
    import collections
    from ktransformers.util.utils import normalize_answer

    original_normalized = normalize_answer(original_text)
    generated_normalized = normalize_answer(generated_text)

    original_tokens = tokenizer.encode(original_normalized)[1:]  # 移除 BOS
    generated_tokens = tokenizer.encode(generated_normalized)[1:]

    common = collections.Counter(original_tokens) & collections.Counter(generated_tokens)
    num_same = sum(common.values())

    if len(original_tokens) == 0 or len(generated_tokens) == 0:
        return float(int(original_tokens == generated_tokens))
    if num_same == 0:
        return 0.0

    precision = num_same / len(generated_tokens)
    recall = num_same / len(original_tokens)
...
def prepare_self_supervised_data(
    ...
    扫描 KV cache 目录获取已有缓存的文档 IDs，从文档池加载原始文本，
    构造自监督重复任务的 prompt。

    Args:
        kv_cache_dir: KV cache 目录路径
        tokenizer: 分词器
        model_type: 模型类型
        dataset_name: 数据集名称
        max_docs: 最大测试文档数（None = 全部）

    Returns:
        questions_data: 自监督任务数据列表，每个元素包含 doc_id, original_text, doc_prompt, doc_tensor, doc_length
        system_tensor: Tokenized system prompt
    """
    import re
    import os

    # 1. 扫描 KV cache 目录获取 doc_ids
    print(f"Scanning KV cache directory: {kv_cache_dir}")
    doc_ids = []
    ...
    for filename in key_files:
        ...
        if match:
            ...
            if doc_id != -1:  # 排除 system prompt
                doc_ids.append(doc_id)

    doc_ids = sorted(set(doc_ids))
    print(f"  Found {len(doc_ids)} document KV caches")
    ...
    print(f"Loading document pool: {pool_path}")
    doc_pool = DocumentPoolLoader(pool_path)

    # 3. 构造自监督 prompt
    REPEAT_SYSTEM_PROMPT = """You are a text repeater.
...
<content>"""
    ...
    system_tokens = tokenizer.encode(REPEAT_SYSTEM_PROMPT, add_special_tokens=False)
    system_tensor = torch.tensor(system_tokens, dtype=torch.long)
    ...
    for doc_id in doc_ids:
        try:
            ...
            # 构造完整 prompt: system + "<content>\n" + doc_text + "\n</content>\n\nEND"
            doc_prompt = f"<content>\n{doc_text}\n</content>\n\nEND"
    ...
    print(f"Prepared {len(questions_data)} self-supervised repeat tasks")
    return questions_data, system_tensor
...
def main(
    ...
    recall_method_display = {
        ...
        RecallMethod.SELF_SUPERVISED: "Self-Supervised Repeat Task",
    }
    ...
    # ========== SELF-SUPERVISED MODE: Word-by-Word Repeat Task ==========
    # 自监督模式需要提前处理，不需要调用 prepare_reflect_data
    if recall_method_enum == RecallMethod.SELF_SUPERVISED:
        print(f"\n{'='*80}")
        print("SELF-SUPERVISED MODE: Word-by-Word Repeat Task")
        print(f"{'='*80}")

        # 准备自监督数据
        questions_data, system_tensor = prepare_self_supervised_data(
            kv_cache_dir=save_path,
            tokenizer=tokenizer,
        ...
        for idx, q_data in enumerate(questions_data):
            ...
            print(f"\n[{idx+1}/{total_docs}] Testing doc_id={doc_id} ({q_data['doc_length']} tokens)")

            # 构造 passages: [system_tensor, doc_tensor, empty_question]
            question_tokens = tokenizer.encode("", add_special_tokens=False)
            question_tensor = torch.tensor(question_tokens, dtype=torch.long)

            passages = [system_tensor, q_data['doc_tensor'], question_tensor]
            doc_ids_for_kv = [-1, doc_id]  # -1 for system, doc_id for document

            # 生成文本（使用 load_kv_and_generate）
            generated_tokens, generated_text, extra_info = load_kv_and_generate(
                model, tokenizer, past_key_values, passages,
                load_path=save_path,
                doc_ids=doc_ids_for_kv,
                max_new_tokens=2048,
                revert_rope=revert_rope,
                reprocess_method='normal',
                rate=1.0,  # 使用完整 KV cache
                preprocess=False,
            )

            # 清理生成文本
            generated_clean = parse_repeat_generation(generated_text)
            original_clean = original_text.strip()

            # 计算 F1 分数
            f1_score = compute_repeat_f1(original_clean, generated_clean, tokenizer)
            exact_match = (normalize_answer(original_clean) == normalize_answer(generated_clean))
            ...
            results.append({
                ...
                'generated_length': len(generated_clean),
                'f1_score': f1_score,
                'exact_match': exact_match,
                'original_text': original_clean[:200] + '...' if len(original_clean) > 200 else original_clean,
                'generated_text': generated_clean[:200] + '...' if len(generated_clean) > 200 else generated_clean,
            })

            print(f"  F1: {f1_score:.4f}, EM: {exact_match}")

        # 输出统计结果
        avg_f1 = total_f1 / total_docs if total_docs > 0 else 0
        em_rate = correct_docs / total_docs if total_docs > 0 else 0
        ...
        print(f"Exact Match Rate: {em_rate:.4f} ({correct_docs}/{total_docs})")
        print(f"{'='*80}")
        ...
        result_file = os.path.join(result_path, 'self_supervised_results.json')
