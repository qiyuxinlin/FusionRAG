...
void KVCache::attn(const ggml_fp16_t* q_in, ggml_fp16_t* output, float* attn_lse, int layer_idx, int q_len, int batch_size, int max_block_num, int* block_table, int* cache_seqlens, int pick_block_num, int init_block_num, int local_block_num, Backend* backend) {
    // 计时
    ...
    // prepare
    ThreadResize(thread_num);
    BatchResize(batch_size);
    ...
    if (block_table == nullptr) {
        ...
        for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
            block_table_before_retrieval_[batch_idx].resize(max_block_num);
    ...
    // retrieval kvcache, get first init_block_num, top pick_block_num
    // similar and last local_block_num blocks. Each task calculates the
    // simlarity of a certain block with the query, then push the block into
    // the priority queue. Finally, the required blocks are pushed into the
    // block_table_after_retrieval_.

    for (int i = 0; i < batch_size; i++) {
        block_table_after_retrieval_[i].resize(max_block_num);
    }
    // This implementation only support batch_size = 1.
    // If batch_size > 1, the implementation should be modified.
    // start_block and end_block need to be modified to start_block[batch_size]
    // and end_block[batch_size].
    if (pick_block_num != -1 ||
        init_block_num + pick_block_num + local_block_num >= max_block_num) {
        int start_block = init_block_num;
        int end_block = max_block_num - local_block_num;
        backend->do_work_stealing_job(
            batch_size * max_block_num, [&](int task_id) {
                int batch_id = task_id / max_block_num;
                int block_id = task_id % max_block_num;
                if (start_block < block_id && block_id < end_block) {
                    return;
                }
                int block_idx =
                ...
                float sim = 0;
                // TODO: caculate sim
                mutex_[batch_id][0]->lock();
                if (top_similar_block_[batch_id].size() < pick_block_num) {
                    top_similar_block_[batch_id].push(
                        std::make_pair(sim, block_idx));
                } else {
                    if (top_similar_block_[batch_id].top().first < sim) {
                        top_similar_block_[batch_id].pop();
                        top_similar_block_[batch_id].push(
                            std::make_pair(sim, block_idx));
                    }
                }
                mutex_[batch_id][0]->unlock();
            });
        for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
            int i = 0;
            ...
            for (; i < local_block_num; i++) {
                block_table_after_retrieval_[batch_idx][i] =
                    block_table_before_retrieval_
                        [batch_idx][max_block_num - init_block_num -
                                    pick_block_num - local_block_num + i];
            }
        }
    } else {
    ...
    for (int i = 0; i < thread_num; i++) {
        thread_cur_head_idx_[i].first = -1;
        thread_cur_head_idx_[i].second = -1;
    }
    ...
    backend->do_work_stealing_job(
        batch_size * config_.kv_head_num * max_block_num, [&](int task_id) {
            ...
            if (cache_seqlens_[batch_id] / config_.block_len == block_id) {
                ...
                attn_with_kvcache_one_block(
                    ...
                    GGML_TYPE_Q8_0, q_q8_0_[batch_id][head_id].data(), seq_len_,
                    0, false, thread_local_attn_mask_[thread_id].data(),
                    GGML_TYPE_Q4_0, 0,
                    k_cache_[layer_id_][head_id][block_idx].data(), 0, nullptr,
                    nullptr, GGML_TYPE_Q4_0, 1,
                    v_cache_[layer_id_][head_id][block_idx].data(), 0, nullptr,
                    nullptr, thread_local_attn_score_[thread_id].data(),
                ...
                attn_with_kvcache_one_block(
                    config_.head_dim, config_.q_head_num / config_.kv_head_num,
                    GGML_TYPE_Q8_0, q_q8_0_[batch_id][head_id].data(), seq_len_,
                    0, true, nullptr, GGML_TYPE_Q4_0, 0,
                    k_cache_[layer_id_][head_id][block_idx].data(), 0, nullptr,
                    nullptr, GGML_TYPE_Q4_0, 1,
                    v_cache_[layer_id_][head_id][block_idx].data(), 0, nullptr,
                    nullptr, thread_local_attn_score_[thread_id].data(),
            ...
            if (batch_id == cur_batch_idx && head_id == cur_head_id) {
                for (int i = 0; i < n_gqa_; i++) {
                    ...
                    //              new_attn_lse));
                    for (int k = 0; k < config_.head_dim; k++) {
                        thread_local_cur_output_fp32_[thread_id][i * config_.head_dim + k] *=
                    ...
                    //              new_attn_lse));
                    for (int k = 0; k < config_.head_dim; k++) {
                        thread_local_output_fp32_[thread_id][i * config_.head_dim + k] *=
                ...
                if (cur_batch_idx != -1) {
                    ...
                    for (int i = 0; i < n_gqa_; i++) {
                        ...
                        //              new_attn_lse));
                        for (int k = 0; k < config_.head_dim; k++) {
                            output_fp32_[cur_batch_idx][cur_head_id][i * config_.head_dim + k] *=
    ...
    backend->do_work_stealing_job(thread_num, [&](int task_id) {
        int cur_batch_idx = thread_cur_head_idx_[task_id].first;
        int cur_head_id = thread_cur_head_idx_[task_id].second;
        ...
        if (cur_head_id != -1) {
            ...
            for (int i = 0; i < n_gqa_; i++) {
                ...
                if (std::abs(attn_lse_[cur_batch_idx][cur_head_id][i]) < 1e-6) {
                    ...
                    //        "output_fp32_[cur_batch_idx][cur_head_id]: %f\n",
                    //        cur_batch_idx, cur_head_id, i,
                    //        output_fp32_[cur_batch_idx][cur_head_id]
                    //                    [i * config_.head_dim]);
                    continue;
...
void attn_with_kvcache_one_block(
    ...
    if (q_type == GGML_TYPE_F16) {
        ...
        llamafile_sgemm(past_kv_len, bsz, head_dim / 32, (block_q4_0*)k_cache,
                        head_dim / 32, (block_q8_0*)q, head_dim / 32,
                        attn_score, past_kv_len, 0, 1, GGML_TASK_TYPE_COMPUTE,
                        k_type, GGML_TYPE_Q8_0, GGML_TYPE_F32, GGML_PREC_DEFAULT);

        // attn = attn * scale
        float scale_factor = 1.0 / std::sqrt(float(head_dim));
        // ggml_vec_scale_f32(bsz * past_kv_len, attn_score, scale_factor);
        for (int k = 0; k < bsz * past_kv_len; k++) {
            attn_score[k] *= scale_factor;
        }
