...
void MLP::forward(const void *input, void *output, Backend *backend) {
    ...
    if (config_.hidden_type ==
        ...
        to_float(input, input_fp32_.data(), config_.hidden_size,
                 config_.hidden_type);
        if (ggml_internal_get_type_traits(config_.gate_type).vec_dot_type ==
            ggml_internal_get_type_traits(config_.up_type).vec_dot_type) {
            from_float(
                input_fp32_.data(), gate_input_.data(), config_.hidden_size,
                ggml_internal_get_type_traits(config_.gate_type).vec_dot_type);
            gate_input_ptr = up_input_ptr = gate_input_.data();
        } else {
            if (config_.hidden_type !=
                ggml_internal_get_type_traits(config_.gate_type).vec_dot_type) {
                from_float(input_fp32_.data(), gate_input_.data(),
                           config_.hidden_size,
            ...
            if (config_.hidden_type !=
                ggml_internal_get_type_traits(config_.up_type).vec_dot_type) {
                from_float(input_fp32_.data(), up_input_.data(),
                           config_.hidden_size,
    ...
    backend->do_work_stealing_job(
        ...
        [&](int task_id) {
            int ith = task_id;
            void *gate_proj_ptr =
                gate_proj_ + ith * config_.stride * config_.hidden_size *
            ...
            void *up_proj_ptr = up_proj_ + ith * config_.stride *
                                               config_.hidden_size *
                                               ggml_type_size(config_.up_type) /
            ...
            if (config_.stride % ggml_blck_size(ggml_internal_get_type_traits(
                ...
                float *intermediate_fp32_ptr =
                    intermediate_fp32_.data() + ith * config_.stride;
                void *down_input_ptr =
                    down_input_.data() +
                    ith * config_.stride *
                        ggml_type_size(
    ...
    if (config_.stride %
        ...
        0) {
        from_float(
            intermediate_fp32_.data(), down_input_.data(),
    ...
    backend->do_work_stealing_job(
        ...
        [&](int task_id) {
            int ith = task_id;
            void *down_proj_ptr =
                down_proj_ + ith * config_.stride * config_.intermediate_size *
            ...
            if (config_.stride % ggml_blck_size(config_.hidden_type) == 0) {
                void *output_ptr =
                    output + ith * config_.stride *
                                 ggml_type_size(config_.hidden_type) /
                                 ggml_blck_size(config_.hidden_type);
                from_float(down_output_ptr, output_ptr, config_.stride,
                           config_.hidden_type);
            }
        },
        nullptr);
    if (config_.stride % ggml_blck_size(config_.hidden_type) != 0) {
        from_float(down_output_.data(), output, config_.hidden_size,
                   config_.hidden_type);
    }
}
