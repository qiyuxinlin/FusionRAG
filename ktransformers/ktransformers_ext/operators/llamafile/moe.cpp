...
MOE::MOE(MOEConfig config) {
    ...
    if (MOE::buffer_ == nullptr) {
        ...
        buffer_size +=
            sizeof(float) * config_.group_max_len * config_.hidden_size;
        buffer_size +=
        ...
        buffer_size +=
            ...
            ggml_blck_size(
                ggml_internal_get_type_traits(config_.up_type).vec_dot_type);
        buffer_size += sizeof(float) * config_.routed_expert_num *
    ...
    offset += config_.routed_expert_num * config_.group_max_len *
              config_.hidden_size *
              ggml_type_size(
                  ggml_internal_get_type_traits(config_.up_type).vec_dot_type) /
              ggml_blck_size(
    ...
    offset += sizeof(float) * config_.routed_expert_num *
              config_.group_max_len * config_.intermediate_size;
...
void MOE::forward_one(int k, const uint64_t *expert_ids, const float *weights,
    ...
    backend->do_work_stealing_job(
        ...
        [&](int task_id) {
            ...
            float *up_output_ptr =
                s_up_output_[expert_idx] + ith * config_.stride;
            llamafile_sgemm(
                config_.stride, 1,
            ...
            if (config_.stride % ggml_blck_size(ggml_internal_get_type_traits(
                                                    config_.down_type)
                                                    .vec_dot_type) ==
                0) {
                ...
                void *down_input_ptr =
                    ...
                    ith * config_.stride *
                        ...
                        ggml_blck_size(
                            ggml_internal_get_type_traits(config_.down_type)
                                .vec_dot_type);
                from_float(intermediate_fp32_ptr, down_input_ptr,
                           config_.stride,
                           ggml_internal_get_type_traits(config_.down_type)
                               .vec_dot_type);
            }
    ...
    backend->do_work_stealing_job(
        ...
        [&](int task_id) {
            ...
            for (int expert_idx = 0; expert_idx < k; expert_idx++) {
                ...
                llamafile_sgemm(config_.stride, 1,
                                config_.intermediate_size /
                                    ggml_blck_size(config_.down_type),
                                down_proj_ptr,
                                config_.intermediate_size /
                                ...
                                config_.intermediate_size /
                                    ggml_blck_size(config_.down_type),
                                down_output_ptr, config_.stride, 0, 1,
                                GGML_TASK_TYPE_COMPUTE, config_.down_type,
                                ggml_internal_get_type_traits(config_.down_type)
                                    .vec_dot_type,
                                GGML_TYPE_F32, GGML_PREC_DEFAULT);
                for (int i = ith * config_.stride;
                     i < (ith + 1) * config_.stride; i++) {
                    s_output_fp32_[i] +=
...
void MOE::forward_many(int qlen, int k, const uint64_t *expert_ids,
        ...
        [&](int i) {
            ...
            if (config_.hidden_type ==
                ...
                gate_input_ptr = up_input_ptr =
                    input + i * config_.hidden_size *
                                ggml_type_size(config_.hidden_type) /
                                ggml_blck_size(config_.hidden_type);
            } else {
                to_float(input + i * config_.hidden_size *
                                     ggml_type_size(config_.hidden_type) /
                                     ggml_blck_size(config_.hidden_type),
                         m_input_fp32_[i], config_.hidden_size,
                         config_.hidden_type);
                if (ggml_internal_get_type_traits(config_.gate_type)
                        .vec_dot_type ==
                    ggml_internal_get_type_traits(config_.up_type)
                        .vec_dot_type) {
                    ...
                    if (config_.hidden_type !=
                        ggml_internal_get_type_traits(config_.gate_type)
                            .vec_dot_type) {
                        from_float(
                            m_input_fp32_[i], m_gate_input_[i],
                            config_.hidden_size,
                            ggml_internal_get_type_traits(config_.gate_type)
                                .vec_dot_type);
                ...
                memcpy(m_local_gate_input_ptr_[expert_ids[i * k + j]] +
                           m_local_pos_[i][j] * config_.hidden_size *
                               ggml_type_size(ggml_internal_get_type_traits(
                                                  config_.gate_type)
                                                  .vec_dot_type) /
                               ggml_blck_size(ggml_internal_get_type_traits(
                                                  config_.gate_type)
                                                  .vec_dot_type),
                       gate_input_ptr,
                       config_.hidden_size *
    ...
    backend->do_work_stealing_job(
        ...
        [&](int task_id) {
            ...
            void *down_proj_ptr =
                down_proj_ + (expert_idx * config_.hidden_size + ith * stride) *
                                 config_.intermediate_size *
                                 ggml_type_size(config_.down_type) /
                                 ggml_blck_size(config_.down_type);
            float *down_output_ptr =
...
void MOE::forward(int qlen, int k, const uint64_t *expert_ids,
    ...
    forward(qlen - forward_len, k, expert_ids + forward_len * k,
            ...
            input + forward_len * config_.hidden_size *
                        ggml_type_size(config_.hidden_type) /
                        ggml_blck_size(config_.hidden_type),
