...
void Linear::warm_up(Backend *backend) {
    ...
    from_float(input_fp32.data(), input.data(), config_.input_size,
               config_.hidden_type);
...
void Linear::forward(const void *input, void *output, Backend *backend) {
    ...
    if (config_.hidden_type ==
        ...
        to_float(input, input_fp32_.data(), config_.input_size,
                 config_.hidden_type);
        from_float(
            input_fp32_.data(), proj_input_.data(), config_.input_size,
            ggml_internal_get_type_traits(config_.proj_type).vec_dot_type);
    ...
    backend->do_work_stealing_job(
        ...
        [&](int task_id) {
            int ith = task_id % nth;
            llamafile_sgemm(
                config_.output_size, 1,
                config_.input_size / ggml_blck_size(config_.proj_type), proj_,
                config_.input_size / ggml_blck_size(config_.proj_type),
                proj_input_ptr,
                config_.input_size / ggml_blck_size(config_.proj_type),
                proj_output_.data(), config_.output_size, ith, nth,
                GGML_TASK_TYPE_COMPUTE, config_.proj_type,
                ggml_internal_get_type_traits(config_.proj_type).vec_dot_type,
                GGML_TYPE_F32, GGML_PREC_DEFAULT);
        },
        nullptr);
    from_float(proj_output_.data(), output, config_.output_size,
               config_.hidden_type);
}