    ...
    for (int i = 0; i < config_.layer_num; ++i) {
        for (int j = 0; j < config_.kv_head_num; ++j) {
            for (int k = 0; k < past_block_num_[i]; ++k) {
                ifs_tensor.read(reinterpret_cast<char*>(k_cache_q4[i][j][k].data()), k_cache_q4[i][j][k].size() * sizeof(block_q4_0));
                ifs_tensor.read(reinterpret_cast<char*>(v_cache_q4[i][j][k].data()), v_cache_q4[i][j][k].size() * sizeof(block_q4_0));
            }
        }
        for (int k = 0; k < past_block_num_[i]; ++k) {
            ifs_tensor.read(reinterpret_cast<char*>(importance_[i][k].data()), importance_[i][k].size() * sizeof(ggml_fp16_t));
        }
    }
    ifs_tensor.close();
    // 计时结束
...
void KVCache::dump_kvcache(int* block_table, int cache_total_len, std::string tensor_file_path, Backend* backend) {
    // 计时
    auto start = std::chrono::high_resolution_clock::now();
    ...
    for (int i = 0; i < config_.layer_num; ++i) {
        for (int j = 0; j < config_.kv_head_num; ++j) {
            for (int k = 0; k < past_block_num; ++k) {
                ...
                ofs.write(reinterpret_cast<const char*>(k_cache_q4[i][j][block_idx].data()), k_cache_q4[i][j][block_idx].size() * sizeof(block_q4_0));
                ofs.write(reinterpret_cast<const char*>(v_cache_q4[i][j][block_idx].data()), v_cache_q4[i][j][block_idx].size() * sizeof(block_q4_0));
            }
        }
        for (int k = 0; k < past_block_num; ++k) {
            int block_idx = block_table[k];
            ofs.write(reinterpret_cast<const char*>(importance_[i][block_idx].data()), importance_[i][block_idx].size() * sizeof(ggml_fp16_t));
        }
    }
    ofs.close();
    // 计时结束
