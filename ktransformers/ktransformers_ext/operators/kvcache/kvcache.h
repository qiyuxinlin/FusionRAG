...
class KVCache {
  public:
    ...
    void get_one_block_fp16(ggml_fp16_t *k_in, ggml_fp16_t *v_in, int layer_id,
    ...
    /*
    kv_cache->get_and_update_fp16((ggml_fp16_t *)k_in,
                                    (ggml_fp16_t *)v_in, layer_id,
                                    (int *)block_table, max_block_num,
                                    (int *)cache_seqlens, q_len, backend);
    */

    void get_and_update_fp16(ggml_fp16_t *k_in, ggml_fp16_t *v_in, int layer_id,
                             int *block_table, int batch_size,
                             int max_block_num, int *cache_seqlens, int q_len,
                             Backend *backend);

    /*
                kv_cache->update_importance(
                    (const ggml_fp16_t *)importance, layer_id, (int
       *)block_table, max_block_num, (int *)offset, width, backend);
    */
    ...
    /*
            kv_cache->attn_with_kvcache(
                (const ggml_fp16_t *)q_in, (const ggml_fp16_t *)k_in,
                (const ggml_fp16_t *)v_in, (ggml_fp16_t *)output,
                (float *)attn_lse, layer_idx, q_len, batch_size, max_block_num,
                (int *)block_table, (int *)cache_seqlens, topk, local, backend);
    */
