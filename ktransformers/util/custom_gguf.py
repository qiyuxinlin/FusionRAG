...
class GGUFLoader:
    ...
    def load_gguf_tensor(self, name, is_gpu=True):
        ...
        if is_gpu:
            dequantize = GGML_DEQUANTIZE_GPU[ggml_name]
        else:
...
def dequantize_q2_k_gpu(data):
    ...
    data_f16 = np.frombuffer(data, dtype=np.float16).reshape(num_blocks, block_size // 2)
    data_u8 = np.frombuffer(data, dtype=np.uint8).reshape(num_blocks, block_size)
    data_f16 = cp.asarray(data_f16)
    data_u8 = cp.asarray(data_u8)
    dmin = data_f16[:, -1].reshape(num_blocks, 1, 1).astype(cp.float32)
    d = data_f16[:, -2].reshape(num_blocks, 1, 1).astype(cp.float32)
    scales = data_u8[:, :16].reshape(num_blocks, 16, 1)
    qs = data_u8[:, 16:80].reshape(num_blocks, 64)
...
def dequantize_q3_k_gpu(data):
    ...
    data_f16 = cp.asarray(data_f16)
    data_u8 = cp.asarray(data_u8)
    d = data_f16[:, -1].reshape(num_blocks, 1, 1).astype(cp.float32)
    bits = cp.unpackbits(data_u8[:, :32].reshape(num_blocks, 32, 1), axis=-1, bitorder="little")
    bits = 4 ^ (bits << 2)
    qs = data_u8[:, 32:32 + 64].astype(cp.int16)
    a, b, c = data_u8[:, 96: 96 + 12].reshape(num_blocks, 3, 4).transpose(1, 0, 2)
    scales = cp.zeros((num_blocks, 4, 4), dtype=cp.uint8)
    scales[:, 0] = (a & 15) | ((c & 3) << 4)
    scales[:, 1] = (b & 15) | (((c >> 2) & 3) << 4)
    scales[:, 2] = (a >> 4) | (((c >> 4) & 3) << 4)
    scales[:, 3] = (b >> 4) | ((c >> 6) << 4)
    scales = scales.reshape(num_blocks, 16, 1).astype(cp.int16)

    return d * (scales - 32) * cp.stack([
        (((qs[:, 00:16] >> 0) & 3) - bits[:, :16, 0]),
        (((qs[:, 16:32] >> 0) & 3) - bits[:, 16:, 0]),
        (((qs[:, 00:16] >> 2) & 3) - bits[:, :16, 1]),
        (((qs[:, 16:32] >> 2) & 3) - bits[:, 16:, 1]),
...
def dequantize_q4_k_gpu(data):
    ...
    data_u8 = cp.asarray(data_u8)
    # Casting to float32 because float16 is very slow on CPU
    scale_factors = data_f16[:, 0].reshape(num_blocks, 1, 1).astype(cp.float32)
    scale_offsets = data_f16[:, 1].reshape(num_blocks, 1, 1).astype(cp.float32)
    qs1 = data_u8[:, 4:16].reshape(num_blocks, 12, 1)
    qs2 = data_u8[:, 16:].reshape(num_blocks, 4, 32)

    # Dequantize scales and offsets (6 bits and 4 + 2 bits)
    factors = scale_factors * cp.concatenate([qs1[:, 0:4] & 0b111111, (qs1[:, 8:] & 15) | ((qs1[:, 0:4] >> 6) << 4)], axis=1)
    offsets = scale_offsets * cp.concatenate([qs1[:, 4:8] & 0b111111, (qs1[:, 8:] >> 4) | ((qs1[:, 4:8] >> 6) << 4)], axis=1)

    # Interleave low and high quantized bits
    qs2 = cp.stack([qs2 & 0xf, qs2 >> 4], axis=2).reshape(num_blocks, 8, 32)
    # Dequantize final weights using scales and offsets
    return factors * qs2 - offsets
...
def dequantize_q5_k_gpu(data):
    ...
    data_f16 = cp.asarray(data_f16)
    data_u8 = cp.asarray(data_u8)
    d = data_f16[:, 0].reshape(num_blocks, 1).astype(cp.float32)
    dmin = data_f16[:, 1].reshape(num_blocks, 1).astype(cp.float32)
    scales = data_u8[:, 4:16].reshape(num_blocks, 12, 1)
    qh = data_u8[:, 16: 16 + 32].reshape(num_blocks, 32, 1)
    qs = data_u8[:, 48: 48 + 128].reshape(num_blocks, 4, 32)

    bits = cp.unpackbits(qh, axis=-1, bitorder="little")
...
def dequantize_q6_k_gpu(data):
    ...
    data_f16 = cp.asarray(data_f16)
    data_u8 = cp.asarray(data_u8)
    data_i8 = cp.asarray(data_i8)
    scales = data_f16[:, -1].reshape(num_blocks, 1).astype(cp.float32)
    # TODO use uint8 and cast later?
    ql = data_u8[:, :128].astype(cp.int16)
    qh = data_u8[:, 128:192].astype(cp.int16)
    sc = data_i8[:, 192:208, cp.newaxis].astype(cp.float32)
