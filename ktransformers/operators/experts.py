...
Version      : 0.1.0
LastEditors  : Azure 
LastEditTime : 2024-08-29 08:50:18
...
class KExpertsCPU(KExpertsBase):
    ...
    def forward(self, input_tensor, expert_ids, weights):
        ...
        # print(expert_ids)
        if input_tensor.size(0)==1 and input_tensor.device != torch.device("cpu"):
...
class KDeepseekV2MoE(BaseInjectedModule, DeepseekV2MoE):
    def forward(self, hidden_states):
        ...
        if sequence_length == 1 and hasattr(self.experts.generate_experts, "submit_for_one_decode") and hidden_states.device != torch.device("cpu"):
