...
class InternLM2SdpaAttention(InternLM2Attention):
    ...
    def forward(
        ...
        attn_output = (
            torch.nn.functional.scaled_dot_product_attention(  # pylint: disable=E1102
                query_states,
                key_states,
                value_states,
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            # TODO joao: remove this `else` after `generate` prioritizes `Cache` objects
                ...
                max_cache_length = torch.tensor(327680, device=input_ids.device)
                cache_length = (
                    past_length
                    if max_cache_length is None
                    else torch.min(max_cache_length, past_length)
                ...
                # past_length = cache_position[0]
                # max_cache_length = torch.tensor(128000, device=input_ids.device)
                # cache_length = (
                #     past_length
                #     if max_cache_length is None
                #     else torch.min(max_cache_length, past_length)
