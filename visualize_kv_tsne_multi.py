#!/usr/bin/env python3
"""
Multi-Method t-SNE Visualization of KV Cache Features

This script compares KV cache feature distributions across multiple methods
using t-SNE for better visualization of local structure.

Usage:
    # Compare 2 methods
    python visualize_kv_tsne_multi.py --sample_ids 0 1 2 \
        --methods no_preprocess bge

    # Compare 3 methods
    python visualize_kv_tsne_multi.py --sample_ids 0 1 \
...
METHOD_COLORS = {
    'no_preprocess': '#1f77b4',  # Blue
    'bge': '#ff7f0e',             # Orange
    'random': '#2ca02c',          # Green
    'repeat_self': '#d62728',     # Red
    'fixed_doc': '#9467bd',       # Purple
...
class MultiMethodKVCacheTSNEAnalyzer:
    """Analyzer for comparing KV Cache across multiple methods using t-SNE"""
    ...
    def analyze_single_sample(self, example_id: int, chunk_id: int,
                            ...
                            perplexity: int = 30):
        """Analyze KV cache for a single sample across multiple methods using t-SNE"""
        print(f"\n{'='*80}")
        ...
        for layer_idx in selected_layers:
            ...
            for method in method_kvs.keys():
                ...
                key_tsne_split[method] = key_tsne[start_idx:end_idx]
                val_tsne_split[method] = val_tsne[start_idx:end_idx]
                start_idx = end_idx
    ...
    def plot_layer_comparison(self, results: Dict, output_dir: str):
        """Plot t-SNE visualization for all layers and methods"""
        ...
        for idx, layer_idx in enumerate(layers):
            ...
            for method in methods:
                tsne_data = layer_data['key_tsne'][method]
            ...
            for method in methods:
                tsne_data = layer_data['val_tsne'][method]
                color = METHOD_COLORS.get(method, 'gray')
        ...
        fig_key.suptitle(f'Key Cache t-SNE - Example {example_id}, Chunk {chunk_id}\nMethods: {", ".join(methods)}',
                         fontsize=14, y=0.998)
        fig_val.suptitle(f'Value Cache t-SNE - Example {example_id}, Chunk {chunk_id}\nMethods: {", ".join(methods)}',
                         fontsize=14, y=0.998)
        ...
        # Save figures
        key_path = output_path / f"tsne_key_example{example_id}_chunk{chunk_id}_{methods_str}.png"
        val_path = output_path / f"tsne_value_example{example_id}_chunk{chunk_id}_{methods_str}.png"
...
def main():
    parser = argparse.ArgumentParser(description='Multi-Method KV Cache t-SNE Visualization')
    ...
    parser.add_argument('--output_dir', type=str, default='./kv_tsne_multi_analysis',
                       help='Output directory for plots')

    args = parser.parse_args()

    print(f"\n{'='*80}")
    print(f"Multi-Method KV Cache t-SNE Analysis")
    print(f"{'='*80}")
    ...
    # Initialize analyzer
    analyzer = MultiMethodKVCacheTSNEAnalyzer(args.cache_dir, args.dataset, args.model_name, args.methods)
