...
Completion API 比较简单，服务器不会存储状态，一次补全只有一次 API 调用。Assistant API 更为复杂，会在服务器中存储用户的状态，一次对话包含多次 API 调用。
...
本项目已经对接好 transformers， 具体代码请见 `server/backend/interfaces/transformers.py`。 
